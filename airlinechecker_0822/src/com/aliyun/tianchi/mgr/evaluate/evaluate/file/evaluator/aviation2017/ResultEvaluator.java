...
public class ResultEvaluator implements Cloneable{
	...
	private double calculateScore(){
		System.out.println("emptyFlightScore:"+emptyFlightScore+"  cancelFlightScore:"+cancelFlightScore+"  flightTypeChangeScore:"+flightTypeChangeScore+"  swapFlightScore:"+swapFlightScore 			
				+"  connectFlightStraightenScore:"+connectFlightStraightenScore+" totalFlightDelayScore:"+totalFlightDelayScore+"  totalFlightAheadScore:"+totalFlightAheadScore+"  passengerCancelScore:"+passengerCancelScore+"  passengerDelayScore:"+passengerDelayScore+"  signChangePassengerDelayScore:"+signChangePassengerDelayScore);
	...
	private void judgeLegalityOfSignChange(ResultFlight resultFlight, Map<String, List<Triple<Integer, Integer, Long>>> passTransInfo){
		...
		while (iter.hasNext()) {
			...
			if(signChangeResultFlight.getStartDateTime().getTime()
				...
				constraintViolationNum += 1;
				System.out.println("签转旅客只能延误 "+flightId+" -> "+signChangeFlightId+" "+signChangeResultFlight.getStartDateTime()+" "+originFlight.getStartDateTime());
				isFeasible = false;
			}
		...
		if(resultFlight.isCancel()) { //航班取消签转, 包含普通航班取消和联程拉直航班取消（只能签转普通旅客）
			...
			if(totalSignChangePassNum > originFlight.getNormalPassengerNum() - transferCancelPassNum){//签转旅客数量不能大于剩余的普通旅客数量
				System.out.println("签转旅客数量不能大于剩余的普通旅客数量 "+flightId+" "+totalSignChangePassNum+" "+originFlight.getNormalPassengerNum()+" "+transferCancelPassNum);
				constraintViolationNum += 1;
				isFeasible = false;
			}
		}
		...
		for(int index = 0; index < resultFlightList.size(); ++index){
			...
			if(!inputData.getAdjustTimeWindow().isInAdjustTimeWindow(originFlight.getStartDateTime().getTime())){//处于非调整窗口
				...
				if(resultFlight.isCancel()){
					...
					transferCancelCost += transferCancelPassNum;
					...
					missTransferCost += transferFailPassNum;
					...
					if(totalPassengerNum + totalSignInChangePassNum > seatNum){//签转旅客到某航班，必须满足该航班的座位数限制
						System.out.println("签转旅客到某航班，必须满足该航班的座位数限制 : f_id "+flightId+" total_pss_num "+totalPassengerNum+
								" total_signToNum "+totalSignInChangePassNum+" seatNum "+seatNum);
						constraintViolationNum += 1;
						isFeasible = false;
					}
				}
			...
			//System.out.println(resultFlight.getFlightId());
			if(resultFlight.getFlightId().equals("159")||resultFlight.getFlightId().equals("216")) {  //测试联程乘客cancel算几截
				System.out.println(resultFlight.getFlightId()+" cancel_cost:"+passengerCancelScoreSub);
			}
		}
